...
const CurrentComp = props => {
  // useEffect(() => {
  //     SetsubLoading(true)
  //     props.CurrentOrders().then(respo => {
  ...
  useEffect(() => {
    ...
    props.OrderReducer.currentOrders.length > 0 && init();
  }, [props.OrderReducer.currentOrders]);
  return (
    <View style={styles.subitemStyle}>
      {/* {!props.appReducer.loading &&  ( */}
      <View style={{padding: 10, flex: 1}}>
        {props.OrderReducer.currentOrders.length > 0 && (
          <Animated.FlatList
            data={props.OrderReducer.currentOrders}
        ...
        {props.OrderReducer.currentOrders.length < 1 && (
          <NoDataComp NoDataText={lang.order.nocurrentdata} />
        )}
      </View>
      {/* )} */}
      {props.appReducer.loading && (
...
export default connect(mapStateToProps, mapDispatchToProps)(CurrentComp);
