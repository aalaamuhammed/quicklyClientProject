...
const AddCreditScreen = props => {
  ...
  useEffect(() => {
    if (props.appReducer.backendErrors.name)
      setError('name', {
        type: 'manual',
        message: props.appReducer.backendErrors.name[0],
      });
    if (props.appReducer.backendErrors.number)
      setError('number', {
        type: 'manual',
        message: props.appReducer.backendErrors.number[0],
      });
    if (props.appReducer.backendErrors.expire_date)
      setError('month', {
        type: 'manual',
        message: props.appReducer.backendErrors.expire_date[0],
      });
    if (props.appReducer.backendErrors.expire_date)
      setError('year', {
        type: 'manual',
...
const mapDispatchToProps = (dispatch, getState = store.getState) => ({
  ...
  addCreditCard: data => CreditsActions.addCreditCard(data)(dispatch, getState),
});
...
export default connect(mapStateToProps, mapDispatchToProps)(AddCreditScreen);
