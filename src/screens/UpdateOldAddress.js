...
const UpdateAddressScreen = props => {
  ...
  const onSubmit = data => {
    props.updateAddress({
      data: {
        ...data,
        id: props.AddressReducer.oneAddress.id,
        // lat: position.latitude,
        // long: position.longitude,
      },
      redirectToMainScreen: () => props.navigation.navigate('Addresses'),
    });
  };
  ...
  useEffect(() => {
    ...
    setValue('address', props.AddressReducer.oneAddress.address);
    setValue('building_no', props.AddressReducer.oneAddress.building_no);
    setValue('city_id', props.AddressReducer.oneAddress.city.id);
    setValue('flat_no', props.AddressReducer.oneAddress.flat_no);
    setValue('floor_no', props.AddressReducer.oneAddress.floor_no);
    setValue('landline', props.AddressReducer.oneAddress.landline);
    setValue('phone', props.AddressReducer.oneAddress.phone);
    setValue('street', props.AddressReducer.oneAddress.street);
    setValue('set_default', props.AddressReducer.oneAddress.set_default);
    SetsubLoading(false);
    setLoadingSub(false);

    setValue('city_id', props.AddressReducer.oneAddress.city.id);
    setValue('governorate_id', props.AddressReducer.oneAddress.governorate.id);
    setCity([props.AddressReducer.oneAddress.governorate.id]);
    setArea([props.AddressReducer.oneAddress.city.id]);
    setValue('lat', parseFloat(props.AddressReducer.oneAddress.lat));
    setValue('long', parseFloat(props.AddressReducer.oneAddress.long));

    props.LoadGovernorates().then(res => {
      settingAreas(props.AddressReducer.oneAddress.governorate.id);
    });
  }, []);
  const sendDataToParent = index => {
    SetPosition(index);
    setValue('lat', index.latitude, {shouldValidate: true, shouldDirty: true});
    setValue('long', index.longitude, {
      shouldValidate: true,
      shouldDirty: true,
    });
    SetVisable(false);
  ...
  return (
    <View nestedScrollEnabled={true} style={styles.mainContaier}>
      {props.AddressReducer.oneAddress && (
        <MapModal
          Visible={Visible}
      ...
      <View
            ...
            <View style={styles.subView}>
              ...
              <View style={{justifyContent: 'center', flexDirection: 'row'}}>
                <FormButton
                  ...
                  onPress={() => {
                    props
                      .deleteAddress(props.AddressReducer.oneAddress.id)
                      .then(respo => {
        ...
        <View
          ...
          <View style={{paddingHorizontal: 10, maxWidth: '60%'}}>
            ...
            {position !== null && (
              <Text style={styles.currentPosition}>
                {position.governorate + ' , ' + position.city}
              </Text>
            )}
          ...
          <FormButton
            textStyles={{color: '#fff', fontSize: 10}}
            styleprops={{
              borderColor: 'transparent',
              backgroundColor: config.colors.mainBgColor,
              paddingVertical: 0,
...
const mapDispatchToProps = (dispatch, getState = store.getState) => ({
  ...
  deleteAddress: data => AddressActions.deleteAddress(data)(dispatch, getState),
  updateAddress: data => AddressActions.updateAddress(data)(dispatch, getState),
});
